...
function App() {
  ...
  // Check if we are currently in "Shared" mode via URL
  const isSharedMode = new URLSearchParams(window.location.search).get('mode') === 'shared';
    ...
    <div className="App">
      {view === 'admin' ? (
        <>
          ...
          <button className="btn-submit" style={{marginTop: '20px'}} onClick={() => setView('public')}>Preview Form</button>
        </>
      ) : (
        <>
          <ReviewForm config={formConfig} />
          
          {/* THE FIX: Only show back button if NOT in shared mode */}
          {!isSharedMode && (
            <button 
              className="btn-remove" 
              onClick={() => setView('admin')} 
              style={{marginTop: '20px', fontSize: '1rem', border: '1px solid var(--accent)', color: 'var(--accent)', padding: '10px'}}
            >
              ← Back to Editor
            </button>
          )}
